...
public protocol PagerViewDataSource: NSObjectProtocol {
    ...
    func numberOfItems(in pagerView: PagerView) -> Int
    ...
    func pagerView(_ pagerView: PagerView, cellForItemAt index: Int) -> UICollectionViewCell
...
public protocol PagerViewDelegate: NSObjectProtocol {
    ...
    optional func pagerView(_ pagerView: PagerView, shouldHighlightItemAt index: Int) -> Bool
    
    /// Tells the delegate that the item at the specified index was highlighted.
    @objc(pagerView:didHighlightItemAtIndex:)
    optional func pagerView(_ pagerView: PagerView, didHighlightItemAt index: Int)
    
    /// Asks the delegate if the specified item should be selected.
    @objc(pagerView:shouldSelectItemAtIndex:)
    optional func pagerView(_ pagerView: PagerView, shouldSelectItemAt index: Int) -> Bool
    
    /// Tells the delegate that the item at the specified index was selected.
    @objc(pagerView:didSelectItemAtIndex:)
    optional func pagerView(_ pagerView: PagerView, didSelectItemAt index: Int)
    ...
    optional func pagerView(_ pagerView: PagerView, willDisplay cell: UICollectionViewCell, forItemAt index: Int)
    ...
    optional func pagerView(_ pagerView: PagerView, didEndDisplaying cell: UICollectionViewCell, forItemAt index: Int)
    
    /// Tells the delegate when the pager view is about to start scrolling the content.
    @objc(pagerViewWillBeginDragging:)
    optional func pagerViewWillBeginDragging(_ pagerView: PagerView)
    
    /// Tells the delegate when the user finishes scrolling the content.
    @objc(pagerViewWillEndDragging:targetIndex:)
    optional func pagerViewWillEndDragging(_ pagerView: PagerView, targetIndex: Int)
    
    /// Tells the delegate when the user scrolls the content view within the receiver.
    @objc(pagerViewDidScroll:)
    optional func pagerViewDidScroll(_ pagerView: PagerView)
    
    /// Tells the delegate when a scrolling animation in the pager view concludes.
    @objc(pagerViewDidEndScrollAnimation:)
    optional func pagerViewDidEndScrollAnimation(_ pagerView: PagerView)
    
    /// Tells the delegate that the pager view has ended decelerating the scrolling movement.
    @objc(pagerViewDidEndDecelerating:)
    optional func pagerViewDidEndDecelerating(_ pagerView: PagerView)
    
}

@IBDesignable
open class PagerView: UIView, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    
    // MARK: - Public properties
    
    /// The object that acts as the data source of the pager view.
    @IBOutlet open weak var dataSource: PagerViewDataSource?
    
    /// The object that acts as the delegate of the pager view.
    @IBOutlet open weak var delegate: PagerViewDelegate?
    ...
    @IBInspectable
    open var automaticSlidingInterval: TimeInterval = 0.0 {
        didSet {
            self.cancelTimer()
    ...
    /// The item size of the pager view. When the value of this property is PagerView.automaticSize, the items fill the entire visible area of the pager view. Default is PagerView.automaticSize.
    @IBInspectable
    ...
    /// An unsigned integer value that determines the deceleration distance of the pager view, which indicates the number of passing items during the deceleration. When the value of this property is PagerView.automaticDistance, the actual 'distance' is automatically calculated according to the scrolling speed of the pager view. Default is 1.
    @IBInspectable
    ...
    @objc
    open var transformer: PagerViewTransformer? {
        didSet {
    ...
    open func registerCell(class:AnyClass) {
...
//    }
    ...
    public func register<T: UICollectionViewCell>(nib: UINib?, forCellWithClass name: T.Type) {
        collectionView.register(nib, forCellWithReuseIdentifier: String(describing: name))
    }
    
    public func dequeueReusableCell<T>(withClass name: T.Type, for index: Int) -> T {
    ...
    fileprivate func startTimer() {
        ...
        let timer = Timer.scheduledTimer(timeInterval: automaticSlidingInterval, target: self, selector: #selector(self.flipNext(sender:)), userInfo: nil, repeats: true)
...
extension PagerView {
    ...
    /// Requests that PagerView use the default value for a given size.
